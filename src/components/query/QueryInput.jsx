...
export function QueryInput({ onSubmit, isProcessing, fileCount }) {
  ...
  return (
    <div className="space-y-3 flex-shrink-0">
      ...
      <div className={`relative rounded-2xl transition-all duration-200 bg-white
        ${isProcessing
      ...
      >
        ...
        <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition-all duration-200 border
          ...
          >
            {isProcessing ? (
              <><span className="w-3.5 h-3.5 rounded-full border-2 border-brand-200 border-t-white animate-spin" />Processing</>
            ) : (
              <><Send size={14} className={hasContent && !isProcessing ? 'text-brand-100' : 'text-slate-400'} />Analyse</>
            )}
          </button>
        </div>
      </div>
